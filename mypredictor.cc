...
#ifdef PREDSTRIDE
if((LOGSTR != 0) && (NBWAYSTR != 0) && (LOGSTRIDE != 0)){
  getPredStride (U, predicted_value, seq_no);
}
#endif
#ifdef PREDVTAGE
if((LOGBANK != 0) && (NBBANK != 0) && (NHIST != 0)){
...
#define KB_16
 ...
 NHIST		=  6	;
  	...
  	HL = new int[NHIST + 1];
	HL[0] = 0; 	HL[1] = 0; 	
	for(int j=2;j<=NHIST;j++){
		double ALPHA=pow(double(LMAX-0.5)/L1,double(1.0/(NHIST-2)));
		HL[j]=int(pow(ALPHA,j-2)*L1+0.5);
		// printf("HL[%d] = %d\n", j,HL[j]);
		// ghrarr[i][j]=ghr_cont;
	}
	LDATA = new longdata[3 * BANKDATA];
	for(int i=0;i<3 * BANKDATA;i++){
		LDATA[i].data = 0;
		LDATA[i].u = 0;
	}
	Vtage = new vtentry[PREDSIZE];
	for(int i=0;i<PREDSIZE;i++){
		Vtage[i].conf = 0;
		Vtage[i].hashpt = 0;
		Vtage[i].tag = 0;
	...
	for(int i=0;i<MAXINFLIGHT;i++){
		Update[i].GI = new uint32_t[NHIST + 1];
		Update[i].GTAG = new uint32_t[NHIST + 1];
		if(NBWAYSTR != 0){
		Update[i].B = new int[NBWAYSTR];
		Update[i].TAGSTR = new int[NBWAYSTR];
		}
	}
	if(NBWAYSTR != 0){
	STR = new strdata[NBWAYSTR * (1 << LOGSTR)];
	}

	// HL = new int[NHIST+1];
	// HL[0] = 0; 	HL[1] = 0; 	HL[2] = 3; 	HL[3] = 7; 	HL[4] = 15; HL[5] = 31; HL[6] = 63; HL[7] = 90; HL[8] = 127;
	// HL[NHIST + 1] = { 0, 0, 3, 7, 15, 31, 63, 90, 127 }
#endif
}
...
endPredictor ()
{
		...
		for(int i=0;i<MAXINFLIGHT;i++){
			delete[] Update[i].GI;
			delete[] Update[i].GTAG;
			delete[] Update[i].B;
			delete[] Update[i].TAGSTR;
		}
